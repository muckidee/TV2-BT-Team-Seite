<?php
/**
 * TV2 Author Rating - Sidebar Display Only
 * Uses existing functions from author-rating-system.php
 */

if ( ! defined( "ABSPATH" ) ) exit;

// Inline CSS for sidebar rating
function tv_sidebar_rating_css() {
    
    ?>
    <style id="tv-sidebar-rating-css">
    ...
    @media(max-width:767px){.tv-author-rating-box{padding:12px}.tv-star-btn{width:32px;height:32px}}
    </style>
    <?php
}
add_action("wp_head", "tv_sidebar_rating_css");

// AJAX handler to get rating data by slug
function tv_sidebar_get_rating_data() {
...
add_action("wp_ajax_tv_sidebar_get_rating", "tv_sidebar_get_rating_data");
add_action("wp_ajax_nopriv_tv_sidebar_get_rating", "tv_sidebar_get_rating_data");

// JavaScript for sidebar rating
function tv_sidebar_rating_script() {
    
    $nonce = wp_create_nonce("tv_rating_nonce");
    $ajax_url = admin_url("admin-ajax.php");
    ?>
    <script id="tv-sidebar-rating-js">
    document.addEventListener("DOMContentLoaded", function() {
        var authorContent = document.querySelector(".author-content");
        if (!authorContent) return;
        
        var authorLink = document.querySelector(".author-card a[href*=\"/team/\"]");
        ...
        fetch(ajaxUrl + "?action=tv_sidebar_get_rating&slug=" + encodeURIComponent(authorSlug) + "&nonce=" + nonce)
...
add_action("wp_footer", "tv_sidebar_rating_script");
